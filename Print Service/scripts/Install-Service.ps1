...
<#
.SYNOPSIS
    Registers LocalPrintServerService as a Windows service configured to
    start automatically at boot and restart on unexpected failure.
...
.PARAMETER BinPath
    Absolute path to the published LocalPrintServerService.exe.

.PARAMETER ServiceName
    Windows service identifier. Defaults to 'KempISPrintingService'.

.PARAMETER DisplayName
    Human-readable name shown in services.msc. Defaults to
    'Kemp IS Printing Service'.

.PARAMETER Description
    Service description shown in services.msc.

.EXAMPLE
    .\Install-Service.ps1 -BinPath "C:\KempISPrintingService\LocalPrintServerService.exe"
#>
...
param(
    ...
    [Parameter()]
    [string]$ServiceName = 'KempISPrintingService',

    [Parameter()]
    [string]$DisplayName = 'Kemp IS Printing Service',

    [Parameter()]
    [string]$Description = 'Local HTTP-to-print bridge. Accepts PDFs on 127.0.0.1:9000 and forwards them to installed Windows printers via SumatraPDF.'
)
