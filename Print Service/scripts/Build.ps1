...
<#
.SYNOPSIS
    Publishes LocalPrintServerService into <repo-root>/bin as a self-contained,
    single-file executable.

.DESCRIPTION
    Wraps `dotnet publish` with the project's standard configuration
    (Release, single-file, self-contained, win-x64, embedded PDB).
...
$projectPath = Join-Path $repoRoot 'LocalPrintServerService\LocalPrintServerService.csproj'
...
$exePath = Join-Path $resolvedOutput 'LocalPrintServerService.exe'
